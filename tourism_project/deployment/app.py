...
model_path = os.path.join(os.path.dirname(__file__), "model.joblib")   # complete the code: filename of the trained model saved by train.py (must match the filename you used there)
...
NumberOfFollowups = st.slider("NumberOfFollowups", 1, 10, 3)
...
if st.button("Predict"):
    prob = model.predict_proba(input_data)[:, 1][0] # Access the probability for the positive class (1)
